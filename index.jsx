...
// ============ 配置部分 START ============
// 文章根目录
let ARTICLE_SRC = config.ARTICLE_SRC;
...
app.get(/^\/articles\/(.+?\.html)$/, (req, res, next) => {
    ...
    // file = file.replace(/\.\.\/?/g, '');
    const src = path.join(ARTICLE_SRC, file);
    ...
    fs.readFile(src, (err, content) => {
        ...
        let injectCode = INJECT_CODE.replace(/\{ts\}/g, Date.now());
        if (req.query.test !== undefined) {
            injectCode = injectCode.replace(/editor\./g, 'dist/editor.');
        }
...
app.post('/upload', function (req, res) {
    ...
    //过滤data:URL
    let fileType = 'png';
    const base64Data = imgData.replace(/^data:image\/(\w+);base64,/, function (o, ext) {
        fileType = ext;
        return '';
    });
...
app.post('/save', function (req, res) {
    ...
    if (content && page) {
        const src = path.join(ARTICLE_SRC, page.replace(/^\/articles/, ''));
        if (!fs.existsSync(src)) {
            return res.send({
                code: -1,
                msg: '原始文件不存在'
        ...
        fs.readFile(src, (err, html) => {
            ...
            const isGbk = html.toString().indexOf('�') != -1;
            if (isGbk) {
...
app.get('/toutiao-video-url', function (req, resp) {
    ...
    fetch(originUrl, TOUTIAO_OPTS).then(res => res.text())
        ...
        .then(res => res.json())
        .then(json => {
            const videoId = /tt-videoid='([^']+)'/.test(json.data.content) && RegExp.$1;
...
app.get('/toutiao-video-poster', function (request, response) {
    fetch(request.query.url, TOUTIAO_OPTS).then(res => res.text())
        .then(text => {
        ...
        .then(res => res.json())
        .then(json => {
            const posterURL = /tt-poster='([^']+)'/.test(json.data.content) && RegExp.$1;
...
if (config.ORIGIN_ARTICLE_PATH) {
    ...
    app.all('*', function (req, res, next) {
        ...
        const options = {
            method: req.method,
            url: config.ORIGIN_ARTICLE_PATH + url,
            qs: req.query,
            json: true,
            body: req.body,
            headers: req.headers
